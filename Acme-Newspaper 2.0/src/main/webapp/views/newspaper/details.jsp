...
<div align="center"><h1><span style="text-decoration: underline">${newspaper.title}</span></h1></div>
...
<jstl:choose>
	...
	<jstl:otherwise>
		<security:authorize access="isAnonymous()">
			<div align="center">
				<h2>
					<jstl:out value="${permiss}"></jstl:out>
				</h2>
			</div>
		</security:authorize>
		...
		<security:authorize access="hasRole('USER')">
			...
			<jstl:choose>
				<jstl:when
					test="${newspaper.user.userAccount.username eq username }">
					<display:table name="${newspaper.articles}" id="row"
						requestURI="${requestURI}" pagesize="10" class="displaytag">
						<display:column titleKey="general.title">
							<a href="article/details.do?articleId=${row.id}">${row.title}</a>
